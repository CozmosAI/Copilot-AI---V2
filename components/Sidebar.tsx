...
const Sidebar: React.FC<SidebarProps> = ({ activeSection, onNavigate }) => {
  ...
  const menuItems = [
    ...
    { id: AppSection.AXIS, label: 'Axis AI', icon: <Cpu size={18} />, badge: 'Novo' },
    ...
    <div className="w-72 md:w-64 bg-[#0f172a] text-slate-300 h-full flex flex-col border-r border-white/5 shadow-2xl relative z-20">
      ...
      <div className="p-8 pb-8 shrink-0">
        <div className="flex items-center gap-3 group cursor-pointer">
           <div className="w-10 h-10 rounded-lg bg-gradient-to-br from-blue-600 to-indigo-600 flex items-center justify-center text-white shadow-lg shadow-blue-900/50">
             <AxisLogo size={20} />
           </div>
           <div>
             <h1 className="text-base font-bold text-white tracking-wide">AXIS AI</h1>
             <p className="text-[10px] text-slate-500 font-medium tracking-wider uppercase">Gestão Inteligente</p>
           </div>
        </div>
      </div>
          ...
          return (
            ...
            >
              <div className="flex items-center gap-3">
                <span className={`transition-colors duration-300 ${isActive ? (isAxis ? 'text-indigo-400' : 'text-blue-400') : 'text-slate-500 group-hover:text-slate-300'}`}>
                  {item.icon}
                </span>
                <span className={`text-sm font-medium ${isAxis ? 'tracking-wider font-bold' : ''}`}>{item.label}</span>
              </div>
